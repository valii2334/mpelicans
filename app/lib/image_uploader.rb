...
class ImageUploader
  ...
  attr_accessor :imageable, :uploaded_files

  def initialize(imageable:, uploaded_files:)
    @imageable      = imageable
  ...
  def generate_s3_key(image:)
    "#{imageable.class.name}-#{imageable.id}-#{SecureRandom.uuid}#{File.extname(image.tempfile)}"
  end

  def create_uploaded_image(image:)
    UploadedImage.create(imageable:, s3_key: generate_s3_key(image:))
  end
end
