...
RSpec.describe ImageUploader do
  ...
  let(:expected_key)   { "#{journey_stop.id}-#{random_uuid}#{File.extname(image_file.tempfile)}" }

  subject { described_class.new(journey_stop_id: journey_stop.id, uploaded_files:).run }
